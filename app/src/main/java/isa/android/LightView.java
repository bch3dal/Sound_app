...
public class LightView extends View {
    ...
    public LightView(Context context) {
        ...
        isPeak = new int[fftConts.bufferSize / 2];
        ...
        for (int i = (int) (100 / HzStep); i < buf.length; i++) {
            if (buf[i] > border) {
                ...
                if (j == stp) {
                    ...
                    //各ピークに対して、左の肩を計算
                    for (int k = i - 1; k >= 0; k--) {
                        if (buf[k] / buf[i] >= border2) {
                            if (isPeak[k] == 0)
                                isPeak[k] = 2;
                            if (k >= 1 && (buf[k - 1] > buf[k]))
                                break;
                        } else
                            break;
                    }
                    ...
                    for (int k = i + 1; k < buf.length; k++)
                        if (buf[k] / buf[i] >= border2) {
                            if (isPeak[k] == 0)
                                isPeak[k] = 2;
                            if (k < buf.length - 1 && (buf[k + 1] > buf[k]))
                                break;
                        } else
                            break;
                }
        ...
        for (int i = 0; i < buf.length; i++) {
            switch (isPeak[i]) {
                case 1:
                    ...
                    canvas.drawText((int) (i * HzStep) + "", (float) (i * zoomX + _yAxisX),
                            205, paint);
                    double trueHz = i * HzStep * buf[i], pwTotal = buf[i];
                    int j = i - 1;
                    while (isPeak[j] == 2) {
                        trueHz += j * HzStep * buf[j];
                        pwTotal += buf[j];
                        j--;
                    }
                    j = i + 1;
                    ...
                    canvas.drawText(String.format("%.2f Hz", trueHz / pwTotal), (float) (i * zoomX + _yAxisX),
                            220, paint);
                    break;
                case 2:
                    paint.setColor(Color.rgb(0xFF, 0x77, 0x00));
                    break;
                default:
                    paint.setColor(Color.BLACK);
            }
